...
const app_routes: Routes = [
  ...
  { path: WebsiteLanguage.French, children: [
      { path: '', loadChildren: () => import('./layouts/welcome/welcome.en.module').then(m => m.WelcomeFrModule)},
      { path: 'resume', loadChildren: () => import('./layouts/resume/resume.en.module').then(m => m.ResumeFrModule) },
      { path: '404', loadChildren: () => import('./layouts/error/error.en.module').then(m => m.ErrorFrModule) },
      //This route doesn't work with npm scully:serve, it does work the http-server
      { path: '**', redirectTo: '404' }
    ]},
  { path: 'hi', loadChildren: () => import('./layouts/hi/hi.module').then(m => m.HiModule) },
